using Homework_Class08.Library.Classes;
...
namespace Homework_Class08
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //phoneBook.Add("Pere Css", "078969691");
            //phoneBook.Add("Viki Dvocefka", "08000500456");
            //phoneBook.Add("Dacka Fitness", "071337925");
            //phoneBook.Add("Dario BMW520", "071274989");
            //phoneBook.Add("Goxi Che fach pula", "075323171");

            //Console.WriteLine("Please enter name of babalu");
            //string input = Console.ReadLine();
            //Console.WriteLine(GetNumber(input,phoneBook));
            ...
            Jerry.FavouriteSong = Songs.Where(song => song.Title.StartsWith("B")).ToList();
            Console.WriteLine("Jerry Songs:");
            Jerry.GetFavSongs();
            Console.WriteLine("-----------------------------------------------------------");
           

            Maria.FavouriteSong = Songs.Where(song => song.Length > 360).ToList();
            Console.WriteLine("Maria Songs:");
            Maria.GetFavSongs();
            Console.WriteLine("-----------------------------------------------------------");

            Jane.FavouriteSong = Songs.Where(song => song.Genre == Genre.Rock).ToList();
            Console.WriteLine("Jane Songs:");
            Jane.GetFavSongs();
            Console.WriteLine("-----------------------------------------------------------");

            Stefan.FavouriteSong = Songs.Where(song => song.Length < 180 && song.Genre == Genre.Hip_Hop).ToList();
            ...
            {
                if(person.FavouriteSong.Count >= 4)
                {
        ...
        #region Exercise No.1
        //public static string GetNumber (string input , Dictionary <string , string> phoneBook)
        //    {
        //        if(phoneBook.ContainsKey(input)== true)
        //        {
        //            return $"{input},{phoneBook[input]}";
        //        }
